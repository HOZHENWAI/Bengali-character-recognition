...
from keras.layers import Input, Dense, ZeroPadding2D, Conv2D, BatchNormalization, Activation, MaxPool2D,AveragePooling2D,Flatten
...
def ResNet55O(input_shape, classes):
    """
    ...
    X = Flatten()(X)
    X = dense_block(X, [2048,1024,512])
    #X = Dropout(rate=0.1)(X)

    #
    X = Dense(classes, activation='softmax', name='graph_out' + str(classes))(X)

    model = Model(inputs = X_input, outputs = [X], name = 'ResNet55O')
    return model
